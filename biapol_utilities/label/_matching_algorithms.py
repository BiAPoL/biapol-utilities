...
def max_similarity(label_image_x, label_image_y, similarity_matrix):
    ...
    metric_matrix : 2D-array, float
        ...
        of label_image_x. In other words, the entry similarity_matrix[i, j] corresponds
        to the "similarity" between label i in label_image_x and label j in
        label_image_y

    Returns
    -------
    ND-array
        Relabelled version of input_image_y. Unmatched labels are appended to the set 
        of matched labels.
    ...
    if similarity_matrix.size > 0:
        ...
        ino = np.nonzero(similarity_matrix.max(axis=1)==0.0)[0]  # Find unpaired labels
